...
void MissMap::SetConfig( Config *conf )
{
  ...
  if( conf->KeyExists( "DRCVariant" ) ) drcVariant = conf->GetString( "DRCVariant" );

  drcChannels = new LH_Cache*[numChannels];
  for( ncounter_t i = 0; i < numChannels; i++ )
    {
      drcChannels[i] = dynamic_cast<LH_Cache *>( MemoryControllerFactory::CreateNewController( drcVariant, GetMemory(), GetTranslator() ));
  ...
  mmSets = 256;
  if( conf->KeyExists( "MissMapSets" ) ) mmSets = static_cast<uint64_t>( conf->GetValue( "MissMapSets" ) );

  mmAssoc = 16;
  if( conf->KeyExists( "MissMapAssoc" ) ) mmAssoc = static_cast<uint64_t>( conf->GetValue( "MissMapAssoc" ) );
  ...
  missMapQueueSize = 32;
  if( conf->KeyExists( "MissMapQueueSize" ) ) missMapQueueSize = static_cast<uint64_t>( conf->GetValue( "MissMapQueueSize" ) );

  uint64_t missMapLatency = 10;
  if( conf->KeyExists( "MissMapLatency" ) ) missMapLatency = static_cast<uint64_t>( conf->GetValue( "MissMapLatency" ) );
...
bool MissMap::IssueAtomic( NVMainRequest *req )
{
  ...
  testAddr.SetPhysicalAddress( (req->address.GetPhysicalAddress( ) >> 6) >> 12 ); // 6 = log2(cacheline size), 12 = log2(segment size)
  lineOffset = ((req->address.GetPhysicalAddress( ) >> 6) & 0xFFF) / 64; // 6 = log2(cacheline size), 0xFFF = segment size - 1, 64 = segment size / cacheline size
  lineMask = (uint64_t)(1ULL << lineOffset);

  std::cout << "Address 0x" << std::hex << req->address.GetPhysicalAddress() << " maps to page 0x"
            << testAddr.GetPhysicalAddress( ) << std::dec << " with offset " << lineOffset
            << std::endl;
  ...
  if( missMapQueue.size( ) < missMapQueueSize )
    {
      ...
      *mmReq = *req;
      ...
      creq->address.SetPhysicalAddress( (creq->address.GetPhysicalAddress( ) >> 6) >> 12 ); // Use the PAGE ADDRESS for the miss map.
      creq->owner = this;
...
bool MissMap::RequestComplete( NVMainRequest *req )
{
...
#ifdef DBGMISSMAP
          std::cout << "MissMap read complete. Hit = " << cacheReq->hit << std::endl;
#endif
          ...
          if( cacheReq->hit )
            {
              ...
              if( (*lineMap) & lineMask )
                {
#ifdef DBGMISSMAP
                  std::cout << "Found cacheline in miss map, issuing to DRC." << std::endl;
#endif

                  /* In DRC -- Issue to DRC. */
                  uint64_t chan;

                  cacheReq->originalRequest->address.GetTranslatedAddress( NULL, NULL, NULL, NULL, &chan );
                ...
                {
#ifdef DBGMISSMAP
                  std::cout << "Did not find cacheline; issuing to main memory." << std::endl;
#endif
                  ...
                  *lineMap |= (uint64_t)(1ULL << lineOffset);
                  ...
                  fillCReq->address.SetPhysicalAddress( (fillCReq->address.GetPhysicalAddress( ) >> 6) >> 12 ); // Use the PAGE ADDRESS for the miss map.
                  fillCReq->owner = this;
...
#ifdef DBGMISSMAP
                  std::cout << "Updating miss map entry 0x" << fillCReq->address.GetPhysicalAddress( )
                            << " with bit vector 0x" << std::hex << (*lineMap) << std::dec << std::endl;
#endif
                }
            }
...
#ifdef DBGMISSMAP
              std::cout << "Adding new miss map entry 0x" << fillCReq->address.GetPhysicalAddress( )
                        << " with bit vector 0x" << std::hex << (*lineMap) << std::dec << std::endl;
#endif
            }
          ...
          if( creq->optype == CACHE_EVICT )
            {
              ...
              *evictReq = *req;
              ...
              /* We should be able to delete this allocated memory at this point. */
              delete lineMap;
  ...
  /* MISSMAP_MEMREAD is a tagged request originating from the sequencer, so we aren't the owner. */
...
#ifdef DBGMISSMAP
      std::cout << "MissMap memory access returned, filling DRAM cache. " << std::endl;
#endif
