...
void STeMS::FetchNextUnused( PatternSequence *rps, int count, std::vector<NVMAddress>& prefetchList )
{
  ...
  if( ReconBuf.count( accessOp->programCounter ) )
    {
      ...
      /* 
       * If the access was a prefetch we did that was successful, prefetch more
       * else if the access is something that was not fetched, deallocate the reconstruct buffer. 
       */
  ...
  if( PST.count( triggerOp->programCounter ) )
    {
          ...
          for( uint64_t i = 0; i < rps->size; i++ )
            {
              ...
              if( rps->used[i] ) numUsed++;
              if( rps->fetched[i] ) numFetched++;
            }

          /* If there are enough used out of the fetched values, get some more. */
          if( numUsed >= 2 )
            {
              FetchNextUnused( rps, 4, prefetchList );
            }
      ...
      /* Create new recon buffer by copying the PST entry */
      else
        {
          ...
          /* Mark the address that triggered reconstruction as fetched and used */
      ...
      if( AGT.count( triggerOp->programCounter ) )
        {
          ...
          addressDiff = ((address > ps->address) ? (address - ps->address) : (ps->address - address));
          ...
          if( ( addressDiff / 64 ) < 256 )
            {
              ...
              /* 
               * If a buffer exists and the size exceeds some threshold, but does not
               * match anything in the pattern table, add this buffer to the pattern table
               */
              if( ps->size >= 8 )
                {
