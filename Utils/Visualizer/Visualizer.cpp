 ...
 *  this object's parent will be the NVMain class under which is it generated (i.e., If 
 *  you are using a hierarchy of memories, it won't be the root NVMain class connected
 *  to your simulator/traceMain). This means casting GetParent()->GetTrampoline() to
 *  NVMain will allow you to call any functions there needed for initialize (e.g., GetConfig).
 *  
 *  After initialization, the parent will become whichever NVMObject the request currently 
 *  resides at (e.g., interconnect, rank, bank, etc.).
 */
void Visualizer::Init( )
{
  ...
  /* Create graphLines for each rank and bank, and set the default symbol to idle. */
  ...
  ncycle_t firstOutput = static_cast<ncycle_t>(GetEventQueue( )->GetCurrentCycle( ) / lineLength) + lineLength;
...
/*
 *  Generally nothing happens during atomic issues (in terms of bank activity). This will
 *  call IssueCommand anyways for corner cases where NVMain's atomic issue is being used
 *  to return average latency values and simulating single requests, for example.
 */
...
/*
 *  Hook the IssueCommand. Here we are interested in bank and rank activity. For ranks
 *  we will place an X on the graph for a time of tCMD. For banks, label the bank graph
 *  with a letter corresponding to the current action the bank is undergoing (e.g, ACT,
 *  READ, WRITE, PRE).
 */
  ...
  if( NVMObjectType == NVMClass(Bank) )
      ...
      switch( req->type )
        {
          ...
          case REFRESH:
            ...
            break;
          /* 
           *  Reads and writes are 'delayed' by tRAS and tCWD, respectively, so we
           *  do not create a symbol for these operation. A bus read or write will
           *  be issued once the read or write operation actually places data on 
           *  the bus. This is generated by the bank via RequestComplete, so we will
           *  hook reads and writes in the RequestComplete function.
           */
          case READ:
          case WRITE:
            break;
  ...
  else if( NVMObjectType == NVMClass(Rank) )
    {
      ...
      if( GetEventQueue( )->GetCurrentCycle( ) % lineLength != 0 )
        {
          ncounter_t fillStop = (GetEventQueue( )->GetCurrentCycle( ) % lineLength) - 1; // assumes tCMD = 1
          ncounter_t fillStart = graphLines[graphId].length( );
...
void Visualizer::Cycle( ncycle_t )
{
  ...
  /* Fill all the graph lines with the idle symbol up to the current simulation cycle and print. */
  for( ncounter_t i = 0; i < numRanks; i++ )
    {
      ...
      /* Print rank graph. */
      std::cout << "RANK " << i << " " << graphLines[i * (numBanks+1)].substr(0,lineLength) << std::endl;
      ...
      for( ncounter_t j = 0; j < numBanks; j++ )
        {
          ...
          /* Print bank graph. */
          std::cout << "BANK " << j << " " << graphLines[graphId].substr(0,lineLength) << std::endl;
        }
    }


  /* Schedule the event for the next visualization print cycle. */
  GetEventQueue( )->InsertEvent( EventCycle, this, GetEventQueue()->GetCurrentCycle() + lineLength ); 
